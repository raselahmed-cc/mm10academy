...
class TCB_Utils {
	...
	/**
	 * Send JSON response or return data
	 *
	 * @param mixed $data   Data to send or return
	 * @param bool  $output Whether to output JSON (true) or return data (false)
	 *
	 * @return mixed Returns data if $output is false, otherwise sends JSON and exits
	 */
	public static function maybe_send_json( $data, $output = true ) {
		// Early return if not outputting
		if ( ! $output ) {
			return $data;
		}

		// Use JSON_UNESCAPED_UNICODE to preserve emojis instead of encoding them as \uXXXX
		// The third parameter of wp_send_json() was added in WordPress 5.6
		// For WordPress 4.8 - 5.5 compatibility, we need to handle encoding manually
		if ( version_compare( get_bloginfo( 'version' ), '5.6', '>=' ) ) {
			wp_send_json( $data, null, JSON_UNESCAPED_UNICODE );
		} else {
			// Manual JSON response for older WordPress versions
			// This mirrors what wp_send_json() does internally, but with JSON_UNESCAPED_UNICODE
			@header( 'Content-Type: application/json; charset=' . get_option( 'blog_charset' ) );
			echo wp_json_encode( $data, JSON_UNESCAPED_UNICODE );

			// Use wp_die() instead of exit for better WordPress integration (allows hooks, testing, etc.)
