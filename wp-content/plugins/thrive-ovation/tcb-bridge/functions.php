...
function tvo_tcb_frontend_enqueue_scripts() {
	...
	// check dependency 'tve_frontend_post' is registered or not.
	if ( ! wp_script_is( 'tve_frontend_post-list', 'registered' ) ) {
		$deps = array_diff( $deps, [ 'tve_frontend_post-list' ] );
	}

	tve_enqueue_script( 'display-testimonials-tcb', TVO_URL . 'tcb-bridge/frontend/js/display-testimonials-tcb' . \TCB_Utils::get_js_suffix(), $deps );
}
