...
class TVO_REST_Social_Media_Controller extends TVO_REST_Controller {
	...
	public function import_testimonial( $request ) {
		...
		$response = array(
			'code'    => 0,
			'message' => __( 'Please enter a valid X or Twitter comment URL', 'thrive-ovation' ),
		);

		if ( 
			strpos( $social_media_url, 'twitter.com' ) !== false ||
			strpos( $social_media_url, 'x.com' ) !== false
		) {
