					...
					<div class="<?php echo esc_attr( $data_alignment ); ?>">
						...
						<div class="uabb-day-new <?php echo esc_attr( $day_align_class ); ?>">
							<div class="uabb-events-new">
								...
								<div class="uabb-events-inner-new">
									...
									if ( 'vertical' === $settings->layout ) {
										?>
										<div class="uabb-timeline-date-hide uabb-date-inner"><div class="inner-date-new"><p><?php echo wp_kses_post( ( 'date' === $settings->items[ $i ]->card_label_type ) ? apply_filters( 'uabb_timeline_the_date_format', $settings->items[ $i ]->date, get_option( 'date_format' ), $settings->items[ $i ]->date, 'custom' ) : $settings->items[ $i ]->label_text ); ?></p></div>
										</div>
									<?php } ?>
						...
						<?php if ( 'vertical' === $settings->layout && 'center' === $settings->orientation ) { ?>
							<div class="uabb-timeline-date-new">
								<div class="uabb-date-new"><div class="inner-date-new"><div><?php echo wp_kses_post( ( 'date' === $settings->items[ $i ]->card_label_type ) ? apply_filters( 'uabb_timeline_the_date_format', $settings->items[ $i ]->date, get_option( 'date_format' ), $settings->items[ $i ]->date, 'custom' ) : $settings->items[ $i ]->label_text ); ?></div></div></div>
							</div>
						<?php } ?>
					</div>
				</div>
				...
				if ( 'custom' === $settings->content_type ) {
					...
					for ( $i = 0; $i < $count; $i++ ) {
						...
						<div class="uabb-timeline-item-<?php echo esc_attr( $i ); ?>">
							<?php
							$date = ( 'date' === $settings->items[ $i ]->card_label_type ) ? apply_filters( 'uabb_timeline_the_date_format', $settings->items[ $i ]->date, get_option( 'date_format' ), $settings->items[ $i ]->date, 'custom' ) : $settings->items[ $i ]->label_text;
