...
class UabbBusinessReview extends FLBuilderModule {
	...
	public function get_google_api_call( $settings ) {
		...
		if ( 'yes-new' === $google_status_code ) {
			...
			// Extract your custom returned values.
			$api_key      = isset( $filtered['key'] ) ? $filtered['key'] : $api_key;
			$place_id     = isset( $filtered['placeid'] ) ? $filtered['placeid'] : $place_id;
			$languagecode = isset( $filtered['language'] ) ? $filtered['language'] : '';

			$url = "https://places.googleapis.com/v1/places/$place_id?fields=id,displayName,formatted_address,reviews&key=$api_key&languageCode=$languagecode";
