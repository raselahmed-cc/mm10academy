...
class UABBRegistrationFormModule extends FLBuilderModule {
	...
	public function enqueue_scripts() {
		...
		if ( isset( $settings->uabb_turnstile_toggle ) && 'show' === $settings->uabb_turnstile_toggle ) {
		...
		if ( isset( $_POST['data'] ) && $allow_register ) {
			...
			if ( isset( $settings->uabb_turnstile_toggle ) && 'show' === $settings->uabb_turnstile_toggle ) {
				...
				if ( empty( $turnstile_response ) ) {
					$error['turnstile'] = __( 'Please complete the Turnstile verification.', 'uabb' );
				} else {
					// Get Turnstile settings from global UABB settings.
					...
					if ( empty( $turnstile_secret_key ) ) {
						$error['turnstile'] = __( 'Turnstile secret key is not configured.', 'uabb' );
					} else {
						$client_ip = self::get_client_ip();
						...
						if ( 200 !== (int) $response_code ) {
							$error['turnstile'] = __( 'Cannot connect to Turnstile verification server.', 'uabb' );
						} else {
							$body   = wp_remote_retrieve_body( $response );
							$result = json_decode( $body, true );

							if ( ! $result['success'] ) {
								$error['turnstile'] = __( 'Turnstile verification failed. Please try again.', 'uabb' );
							}
						}
					}
				}
