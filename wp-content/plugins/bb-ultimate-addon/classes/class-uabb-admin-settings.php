...
final class UABBBuilderAdminSettings {
	...
	public static function init_hooks() {
		...
		if ( 'yes' === get_option( 'uabb_usage_optin', false ) ) {
			add_action( 'shutdown', __CLASS__ . '::uabb_check_modules_data_usage' );
		}
	}
	/**
	 * Adds the admin menu and enqueues CSS/JS if we are on
	...
	public static function render_nav_items() {
		...
		$items['uabb-turnstile']        = array(
			'title'    => __( 'Turnstile Settings', 'uabb' ),
			'show'     => ! is_network_admin() || ! FLBuilderAdminSettings::multisite_support(),
			'priority' => 512,
		);
	...
	public static function save() {
		...
		if ( isset( $_POST['fl-uabb-nonce'] ) && wp_verify_nonce( sanitize_text_field( $_POST['fl-uabb-nonce'] ), 'uabb' ) ) {
			...
			if ( isset( $_POST['uabb-enable-analytics-updates'] ) ) {
				...
				update_option( 'uabb_usage_optin', 'no' );
			}
		...
		if ( isset( $_POST['uabb-turnstile-nonce'] ) && wp_verify_nonce( sanitize_text_field( $_POST['uabb-turnstile-nonce'] ), 'uabb-turnstile-nonce' ) ) {
			$existing_integration = get_option( '_uabb_integration', array() );
			
			if ( isset( $_POST['cloudflare_turnstile_site_key'] ) ) {
				$existing_integration['cloudflare_turnstile_site_key'] = sanitize_text_field( $_POST['cloudflare_turnstile_site_key'] );
			}
			if ( isset( $_POST['cloudflare_turnstile_secret_key'] ) ) {
				$existing_integration['cloudflare_turnstile_secret_key'] = sanitize_text_field( $_POST['cloudflare_turnstile_secret_key'] );
			}
			if ( isset( $_POST['cloudflare_turnstile_theme'] ) ) {
				$valid_themes = array( 'auto', 'light', 'dark' );
				$theme        = sanitize_text_field( $_POST['cloudflare_turnstile_theme'] );
				$existing_integration['cloudflare_turnstile_theme'] = in_array( $theme, $valid_themes, true ) ? $theme : 'auto';
			}
			if ( isset( $_POST['cloudflare_turnstile_size'] ) ) {
				$valid_sizes = array( 'normal', 'compact' );
				$size        = sanitize_text_field( $_POST['cloudflare_turnstile_size'] );
				$existing_integration['cloudflare_turnstile_size'] = in_array( $size, $valid_sizes, true ) ? $size : 'normal';
			}
			
			update_option( '_uabb_integration', $existing_integration );
		}
	...
	public static function uabb_check_modules_data_usage() {
		...
		$transient_key = 'uabbpro_module_usage_data';
		$module_usage  = get_transient( $transient_key );

		if ( false === $module_usage || false === get_option( 'uabbpro_module_usage_data_option' ) ) {
			$filtered_module_usage = self::get_uabb_module_usage();

			set_transient( $transient_key, $filtered_module_usage, MONTH_IN_SECONDS );
			update_option( 'uabbpro_module_usage_data_option', $filtered_module_usage );
		}
	}
