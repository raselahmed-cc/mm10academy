...
class uabb_lessc {
    ...
    protected function injectVariables($args) {
        $this->pushEnv();
        $parser = new uabb_lessc_parser($this, __METHOD__);
    ...
    protected function makeParser($name) {
        $parser = new uabb_lessc_parser($this, $name);
    ...
    protected function newFormatter() {
        $className = "uabb_lessc_formatter_lessjs";
        if (!empty($this->formatterName)) {
            if (!is_string($this->formatterName))
                return $this->formatterName;
            $className = "uabb_lessc_formatter_$this->formatterName";
        }
...
class uabb_lessc_parser {
    ...
    public function __construct($lessc, $sourceName = null) {
        ...
        if (!self::$operatorString) {
            self::$operatorString =
                '('.implode('|', array_map(array('uabb_lessc', 'preg_quote'),
                    array_keys(self::$precedence))).')';

            $commentSingle = uabb_lessc::preg_quote(self::$commentSingle);
            $commentMultiLeft = uabb_lessc::preg_quote(self::$commentMultiLeft);
            $commentMultiRight = uabb_lessc::preg_quote(self::$commentMultiRight);
    ...
    protected function isDirective($dirname, $directives) {
        ...
        $pattern = implode("|",
            array_map(array("uabb_lessc", "preg_quote"), $directives));
    ...
    protected function expressionList(&$exps) {
        ...
        $exps = uabb_lessc::compressList($values, ' ');
        return true;
    }
    ...
    public function propertyValue(&$value, $keyName = null) {
        ...
        $value = uabb_lessc::compressList($values, ', ');
        return true;
    }
    ...
    protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
        ...
        $stop = array("'", '"', "@{", $end);
        $stop = array_map(array("uabb_lessc", "preg_quote"), $stop);
    ...
    protected function literal($what, $eatWhitespace = null) {
        ...
        if (!isset(self::$literalCache[$what])) {
            self::$literalCache[$what] = uabb_lessc::preg_quote($what);
        }
    ...
    protected function to($what, &$out, $until = false, $allowNewline = false) {
        ...
        if (!$this->match('('.$validChars.'*?)'.uabb_lessc::preg_quote($what), $m, !$until)) return false;
...
class uabb_lessc_formatter_classic {
    public $indentChar = "  ";
...
class uabb_lessc_formatter_compressed extends uabb_lessc_formatter_classic {
...
class uabb_lessc_formatter_lessjs extends uabb_lessc_formatter_classic {
    public $disableSingle = true;
